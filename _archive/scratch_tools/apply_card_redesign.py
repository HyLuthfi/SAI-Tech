...
def make_card_html(p):
    wa_msg = f"Halo Admin SAI Tech, saya ingin pesan {p['name']} ({p['price']})"
    import urllib.parse
    wa_url = f"https://wa.me/6282163732969?text={urllib.parse.quote(wa_msg)}"
    return f"""          <!-- {p['name']} -->
          <div class="product-box" data-category="{p['category']}" data-name="{p['search_terms']}">
            ...
            <div class="product-action-row">
              ...
              <a href="{wa_url}" target="_blank" rel="noopener noreferrer" class="btn-pesan">Pesan</a>
            </div>
          </div>"""
...
# Replace CSS
old_css_pattern = re.compile(r'/\* Product Cards Grid \*/.*?/\* Steps Section \*/', re.DOTALL)
...
print("Product cards redesigned successfully with 1:1 image, name, short desc, and Pesan button!")
